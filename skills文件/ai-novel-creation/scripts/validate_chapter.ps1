...
$specificValuePattern = "(?!\s*(\{|无|暂无|没有|待定))\s*[^\r\n\*]+"
...
$hasCausalValidation = $text -match "$causalValidationWord$colonPattern\s*$passWord"
# 主角代价必须写出具体内容，不能用“无”“待定”或模板占位符逃避人物行动后果。
$hasProtagonistCost = $text -match "$protagonistCostWord$colonPattern$specificValuePattern"
# 下一章钩子必须写出具体内容，保证章节结尾能自然牵引后续情节。
$hasNextHook = $text -match "$nextHookWord$colonPattern$specificValuePattern"
